...
async function commitMetadata(repo, hash) {
  ...
  return {
    timestamp: Number(output.subarray(0, separator).toString()),
    message: output.subarray(separator + 1).toString('utf8').replace(/(?:\r?\n)+$/, '')
  };
}
...
async function analyzeRepository(rawOptions) {
  ...
  for await (const hash of createInterface({ input: child.stdout, crlfDelay: Infinity })) {
    ...
    // Follow renames even when their commit message is excluded from the report.
    if (!options.message.test(metadata.message)) continue;
    for (const file of touched) {
      counts.set(file, (counts.get(file) || 0) + 1);
      const fileDetails = details.get(file) || [];
      fileDetails.push({
        hash,
        date: new Date(metadata.timestamp * 1000).toISOString(),
        message: metadata.message
      });
      details.set(file, fileDetails);
    }
    selected += 1;
  ...
  return [...counts].map(([filePath, commits]) => ({
    path: filePath,
    commits,
    details: details.get(filePath)
  }))
